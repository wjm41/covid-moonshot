...
def return_overlap(x1, x2, radius=0.8, n_overlap=3, make_plot=False, verbose=False):
    ...
    # read 2 xyz files
    u_atoms = read_xyz(x1)[0].get_positions()
    v_atoms = read_xyz(x2)[0].get_positions()

    # loop over atom xyz vectors, calculate euclidean distance
    dist_mat = np.empty((len(u_atoms),len(v_atoms)))
    for i, u in enumerate(u_atoms):
        for j,v in enumerate(v_atoms):
...
def main(args):
    ...
    for u in covalent_indices:
        for v in non_covalent_indices:
            overlap = return_overlap('data/covalent/Mpro-x'+u+'_0.xyz', 'data/non_covalent/Mpro-x'+v+'_0.xyz',
                                     radius=args.radius, n_overlap=args.n_overlap, verbose=args.verbose)
            if overlap:
